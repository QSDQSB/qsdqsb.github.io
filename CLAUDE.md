...
| `_pages` | `title` | `layout` (non-default), `permalink` (routable), `map_dataset` (points at a generated `assets/maps/<name>.geojson`, e.g. `voyage-atlas`) |
| `_voyage` | `title`, `date`, `header` | exactly one of: `gallery_name` OR `subgalleries: true`; optional `map:` viewport block (only for `subgalleries:true` voyages); `map_dataset:` only to reference a generated atlas directly (see below) |
...
- **`map_dataset: <name>`** — points `_includes/map.html` at an existing `assets/maps/<name>.geojson` for pages that include the map partial with no `include.dataset` param (e.g. `_portfolio/voyage.html` sets `map_dataset: voyage-atlas` to render the global atlas at `/voyage/`). It is *not* a way to hand-author new marker data: the hand-curated `_data/maps/*.yml` pipeline that used to back this key was dead (no such directory ever existed alongside it) and has been removed — every `<name>.geojson` today is generated by `scripts/geocode-maps.js` from `_voyage`/`_subvoyage` frontmatter, either the global atlas (`voyage-atlas`) or a per-parent atlas (`voyage-<basename>`, for `subgalleries:true` voyages). (If a `subgalleries:true` parent also sets `map_dataset:`, the dataset wins via `page.map_dataset | default: auto_map_id` in the layout, but you almost never want this.)
...
2. Any page with `map_dataset:` set and no `dataset` param passed to the include (e.g. `_portfolio/voyage.html`) — the renderer loads `assets/maps/<dataset>.geojson` directly.

Both paths use the same Leaflet+map.js atlas renderer — every dataset `scripts/geocode-maps.js` generates carries `geojson.properties.kind === 'voyage-atlas'` (tag filter + rich popups).
