...
| `assets/maps/*.geojson` | Generated by `npm run geocode` from voyage/sub-voyage frontmatter (`map:` blocks) via `scripts/geocode-maps.js`. Inspect the frontmatter, not the generated cache. |
...
| `_pages` | `title` | `layout` (non-default), `permalink` (routable), `map_dataset` (points at a GeoJSON dataset; see below) |
| `_voyage` | `title`, `date`, `header` | exactly one of: `gallery_name` OR `subgalleries: true`; optional `map:` viewport block (only for `subgalleries:true` voyages); `map_dataset:` to override the auto-derived dataset id (see below) |
...
- **`map_dataset: <name>`** — points `_includes/map.html` at `assets/maps/<name>.geojson` directly, bypassing the `subgalleries:true` auto-derive. There is no `_data/maps/` YAML backing any more (that hand-curated pipeline was retired) — `<name>` must simply match a GeoJSON file that `scripts/geocode-maps.js` actually writes. **Live usage:** `_portfolio/voyage.html` sets `map_dataset: voyage-atlas` to render the global cross-voyage atlas (`assets/maps/voyage-atlas.geojson`, one feature per `_voyage/*` page). On a `subgalleries:true` voyage you can also set `map_dataset:` to point at a *different* dataset than its own auto-derived `voyage-<slug>.geojson` — the explicit value wins via `page.map_dataset | default: auto_map_id` in the layout, but you almost never want this.
...
map:                             # optional, only meaningful for subgalleries:true.
  ...
  maxZoom: 20
# map_dataset: barcelona         # override the dataset id — points at
                                 # assets/maps/barcelona.geojson instead of
                                 # the auto-derived voyage-<slug>.geojson.
---
```
...
Map block (`_includes/map.html`) has two call sites:
1. `_layouts/gallery.html`, after main content, for any voyage with `subgalleries: true` — the layout passes `dataset="voyage-<basename>"` and the renderer loads the auto-derived `assets/maps/voyage-<basename>.geojson` (one feature per child).
2. `_portfolio/voyage.html`, the `/voyage/` archive page — sets `map_dataset: voyage-atlas` in its own frontmatter (no `dataset=` param needed) to load `assets/maps/voyage-atlas.geojson`, one feature per `_voyage/*` page.

Both are written by `scripts/geocode-maps.js` and share the same atlas renderer (tag filter + rich editorial popups) — `map.js` only implements this one rendering mode; it does not have a separate simple-marker/legacy mode.
