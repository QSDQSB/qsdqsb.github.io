 ...
 *
 * Renders atlas datasets (multi-tag markers, lightweight quick-view popups,
  ...
  function loadAndRenderGeoJSON(map, geojsonPath, container, viewportPreset) {
    ...
    fetch(normalizedPath, { method: 'GET', credentials: 'same-origin' })
      ...
      .then((geojson) => {
        ...
        // Every generated dataset carries kind: 'voyage-atlas' (see
        // scripts/geocode-maps.js), so this is always an atlas render.
        // isAtlasDataset/getMapBoundsPadding stay general-purpose (and
        // tested) rather than hard-coded to that assumption.
        const atlasDataset = isAtlasDataset(geojson);
        renderAtlasMap(map, geojson, container);

        // Effective viewport precedence:
        //   1. viewportPreset (named VIEWPORT_PRESETS entry or an explicit
        //      `viewport` option passed to init() — no current caller feeds
        //      this, it's a general init() capability)
