...
const merged = (slugs, extra = {}) => ({
  ...
  inventory: slugs.map(s => ({ slug: s, file: `${s.toUpperCase()}.jpg`, taken: null, camera: null, formats: ['jpg', 'webp'], processed: '2026-09-24T10:00:00Z', ...(extra[s] || {}) })),
});
...
test('a fully processed, captioned gallery reports no problems', async () => {
  ...
  const [row] = buildStatus({
    ...
    merged: () => merged(['dscf0001', 'dscf0002'], { dscf0002: { camera: 'FUJIFILM X-T5', taken: '2024-05-01T09:00:00' } }),
