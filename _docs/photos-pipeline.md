...
`workers/photos-trigger/`, or the processing workflow. **To add, change or
remove photographs, start with `_docs/photos-recipes.md`**; this is the reference
behind it.
...
| `npm run photos:locate -- --gallery x \| --all [--sheets] [--set slug="place"] [--accept [slugs]]` | Suggests where each photo was taken (a well-known landmark, square, park or inn, else the road) into `_data/photo_locations/`; `--accept` writes suggestions into empty captions. | none (OpenStreetMap, HTTP) |
| `npm run photos:enrich [-- --gallery x] [--dry-run] [--by-time]` | Puts back the Fujifilm maker notes an edited Photos export drops: finds each original's item in Apple Photos, exports the unedited camera file, copies its maker notes and EXIF across with ExifTool. Resumable, memory-guarded, one run at a time. | none (Apple Photos) |
...
### Import into a voyage

Adding frames to a voyage, or a new voyage's photographs.
`npm run photos:ingest` runs the whole of it with one confirmation before
the push; step by step:

1. Export from Photos into the inbox, `~/Desktop/voyage originals`
   (`PHOTOS_INBOX`), in any folders or none; folder names are hints, not
   trusted. Export **as edited** (File → Export): the site publishes your
   crop. That export drops the Fujifilm maker notes; `photos:enrich` puts
   them back afterwards (*Enrich from Apple Photos* below).
   `npm run photos:import -- --dry-run`, then
   `npm run photos:import -- --move`. For each file the gateway:
   - **matches by name**: galleries holding the same frame number,
     confirmed by picture against the original `photos/` holds, since
     frame numbers repeat across years;
   - **else by picture**: the closest photograph in `photos/` by
     fingerprint (48×48 greyscale, correlation ≥ 0.95; true pairs score
     ≥ 0.99, different frames of one beach ≤ 0.76). A match is imported
     under that frame's name, so a renamed export (`IMG_…`,
     `DSCF1797 (1)`) keeps its slug and captions;
   - **verifies** it: decodes whole, carries camera make and capture time,
     same aspect ratio as the original it matches;
   - **adds** it as a new frame when the file sits in a voyage's folder and
     nothing in `photos/` matches. A frame `photos/` already holds is
     refused unless `--replace-originals` (a re-export after an edit; a new
     crop passes when camera and capture time match, reported as a new edit). With
     `--move` a copied file leaves the inbox once the copy is byte-identical.
   Files that match nothing, fail a check, or duplicate another staged file
   stay in the inbox, are listed in the output, and leave only when you say
   so (`--discard`, to the Trash). Sweep them one by one: a wrong export is
   discarded; a frame worth keeping is moved into its voyage's folder in
   the inbox and imported as a new frame.
2. `npm run photos:recollect -- --gallery <voyage>` and read the report:
   - **matched**: frames the gallery already had, and how many of them are
     byte-identical to the bucket's.
   - **new**: frames the gallery did not have.
   - **renamed**: same frame, different name from the bucket's. The Fuji
     writes `DSCF1797.JPG`; the bucket may hold `DSCF1797.jpg`. R2 keys are
...
### Enrich from Apple Photos

An edited export from Photos loses the Fujifilm maker notes (film
simulation, dynamic range, grain, colour chrome, tones, focus and drive
modes, shutter type, stabilisation, shutter count), which the database
wants. `npm run photos:enrich` puts them back into every Fujifilm original
in `photos/` that lacks them (`photos:ingest` runs it for each gallery it
touched):

1. It asks Photos for items with the file's name, in small batches, and
   keeps the one taken at the same second, since frame numbers repeat
   about every 10,000 shots. Only exact-name lookups: a `whose` over dates
   or a name prefix makes Photos load the whole library (it reached 66 GB
   once).
   `--by-time` finds items by capture time alone, for renamed files, by
   reading the library's names and dates once; it also retries earlier
   misses.
2. It exports that item's unedited camera file to
   `.photos-local/collect/enrich-stage/` (Photos itself is never touched).
3. ExifTool copies the maker notes and EXIF into the file in `photos/`;
   the pixels, orientation and dimensions of your edit stay as they are.
   A result that changed shape or still lacks maker notes is restored
   byte for byte.

It needs `brew install exiftool`, and the first run asks macOS to let this
app control Photos. State in `.photos-local/collect/enrich-state.json` (a
stopped run resumes), a lock (one run at a time), Photos restarted past
6 GB, failed lookups and exports retried with a pause and otherwise left
for the next run, a log in `.photos-local/collect/log.txt`. The folder is
named for the retired `photos:collect`.

### A short session

An import does not need a whole voyage at once. Three photos of one
...
2. `photos:recollect -- --gallery <voyage> --push`. The push is the
   backup: from that moment the original is in the bucket, and any
   original it replaced sits in `trash/` for 30 days.
3. Stop.
...
private manifests in the bucket. Anything that was
...
`--accept` writes a caption only while it is empty; one already written is
...
The `photos.compressed` column is vestigial: it marked the site's old
compressed copies while they were replaced by camera originals, and the
processor no longer sends the field, so the Worker writes 0. A later
...
**4. Check.** `npm run photos:status`: the bucket columns fill once the
rclone remote and the bucket answer, and a pushed gallery reads processed
once the Worker and the workflow have run (`gh run list --workflow
photos-process.yml` shows the last run). The Pages build needs nothing: it
...
Only the originals and the authored YAML need keeping; everything else is
in git or rebuilt from the originals.
...
| The site's old `gallery/` images | git history (about 600 MB in every clone, until a history rewrite drops them) | — | never, while git history keeps them |
...
- **Replacements**: R2 keeps no object versions, so `photos:push` runs
  ...
  (a re-export after an edit, or a mistake) is moved to
...
the clone removes it. Push soon after each import, and keep the camera
library itself until `photos:status` shows nothing pending for the voyage.
...
- `npm test` also covers push moving replaced originals to trash,
  `photos:status` row building and orphan classification, the import
  gateway (by name, by picture, refusing to overwrite an original without
  `--replace-originals`), the captions scaffold (text-level append,
  capture-time order, idempotence), the re-collection comparison and
  `--rename`, `photos:locate --accept`, and trash list/restore. The
  recollect and trash CLI tests run real rclone against a local-directory
  remote, and skip when rclone is not installed.
